...
#'
#' Tobler's offpath cost function reduces speed of the Tobler's 'Hiking function' by 0.6 to take into account walking off-path (Tobler, 1993)
#'
...
#'
#' Llobera and Sluckin (2007) metabolic energy expenditure cost function in KJ/(m*kg)
#'
#' @param dem \code{RasterLayer} (raster package). Digital Elevation Model.
#'
#' @param cost_function Cost Function used in the Least Cost Path calculation. Implemented cost functions include 'tobler', 'tobler offpath', 'modified tobler', 'wheeled transport' 'herzog', 'llobera_sluckin' or 'all'. Default is 'tobler'. See Details for more information.
#'
...
create_slope_cs <- function(dem, cost_function = "tobler", neighbours = 16, crit_slope = 12, max_slope = NULL) {
    ...
    cfs <- c("tobler", "tobler offpath", "modified tobler", "wheeled transport", "herzog", "llobera_sluckin", "all")
    ...
    if (cost_function == "llobera_sluckin") {
    ...
    if (cost_function %in% cfs[cfs == "all"]) {
        ...
        slope_stack <- list(slope, slope, slope, slope, slope, slope)
