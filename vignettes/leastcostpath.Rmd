...
The <i>leastcostpath</i> package makes the calculation of conductance surfaces fast and easy. Implementing multiple time- and energy-based cost functions, as well as allowing for user-supplied functions, the <i>leastcostpath</i> aims to be flexible.

These conductance surfaces can be used to calculate least-cost paths, accumulated cost surfaces, amongst others.

This documentation introduces the <i>leastcostpath</i> workflow, and shows you how to apply it to your data. 

Three cases studies are also outlined:
* Calculation of a potential route within the landscape (termed predictive)
* Identification of natural corridors within the landscape
* Explaination of factors governing the location of a known route (termed postdictive)
...
Here, we know that cell 48260 is within the landscape of Sicily. Let's filter to only those that are non-zero (i.e. adjacent/connected).

Given that we used a neighbourhood matrix of 4 when creating the conductanceMatrix, we can see that cell 48260 is connected to 4 different cell. 
...
If we want to crop the conductanceMatrix to a different extent, we can use the \code{crop_cs} function. For this, we need to supply an sf  or terra spatRater object.
...
# 3. Calculate a potential route within the landscape (prediction)

# 4. Identify natural corridor within the landscape

# 5. Explain the factors governing the location of a known route
